...
class ObstaclesPublisher : public rclcpp_lifecycle::LifecycleNode {
 private:
  ...
  rclcpp::Publisher<obstacles_msgs::msg::ObstacleArrayMsg>::SharedPtr
      publisher_;
 ...
 public:
  explicit ObstaclesPublisher(bool intra_process_comms = false)
      : rclcpp_lifecycle::LifecycleNode(
            "send_obstacles", rclcpp::NodeOptions().use_intra_process_comms(
                                  intra_process_comms)) {
  ...
  on_configure(const rclcpp_lifecycle::State& state) {
    ...
    this->declare_parameter("max_timeout", 6);
    ...
    RCLCPP_INFO(this->get_logger(), "map: %s",
                this->get_parameter("map").as_string().c_str());
    RCLCPP_INFO(this->get_logger(), "dx: %f",
                this->get_parameter("dx").as_double());
    RCLCPP_INFO(this->get_logger(), "dy: %f",
                this->get_parameter("dy").as_double());
    RCLCPP_INFO(this->get_logger(), "max_timeout: %ld",
                this->get_parameter("max_timeout").as_int());
    RCLCPP_INFO(this->get_logger(), "n_obstacles: %ld",
                this->get_parameter("n_obstacles").as_int());
    RCLCPP_INFO(this->get_logger(), "no_cylinders: %d",
                this->get_parameter("no_cylinders").as_bool());
    RCLCPP_INFO(this->get_logger(), "no_boxes: %d",
                this->get_parameter("no_boxes").as_bool());
    RCLCPP_INFO(this->get_logger(), "min_size: %f",
                this->get_parameter("min_size").as_double());
    RCLCPP_INFO(this->get_logger(), "max_size: %f",
                this->get_parameter("max_size").as_double());

    this->data.map_name =
        this->get_parameter("map").as_string();  // hexagon, rectangle
    ...
    if (this->data.no_cylinders && this->data.no_boxes) {
      RCLCPP_ERROR(this->get_logger(),
                   "Both no_cylinders and no_boxes are true. At least one of "
                   "them must be false.");
      return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::
          CallbackReturn::ERROR;
    }
    if (this->data.map_name != "hexagon" &&
        this->data.map_name != "rectangle") {
      RCLCPP_ERROR(this->get_logger(),
                   "Map parameter must be either hexagon or rectangle.");
      return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::
          CallbackReturn::ERROR;
    }
    ...
    this->publisher_ =
        this->create_publisher<obstacles_msgs::msg::ObstacleArrayMsg>(
            "/obstacles", qos);

    this->borders_tran_sub_ =
        this->create_subscription<lifecycle_msgs::msg::TransitionEvent>(
            "/send_borders/transition_event", 10,
            std::bind(&ObstaclesPublisher::on_borders_transition_event, this,
                      std::placeholders::_1));

    this->gates_tran_sub_ =
        this->create_subscription<lifecycle_msgs::msg::TransitionEvent>(
            "/send_gates/transition_event", 10,
            std::bind(&ObstaclesPublisher::on_gates_transition_event, this,
                      std::placeholders::_1));
 ...
 private:
  ...
  void gate_callback(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
    for (auto pose : msg->poses) {
      obstacle obs{0.0, pose.position.x,   pose.position.y, 1.0,
                   1.0, obstacle_type::BOX};
    ...
    if (msg->goal_state.label == "deactivating" &&
      ...
      this->gate_sub_ =
          this->create_subscription<geometry_msgs::msg::PoseArray>(
              ...
              std::bind(&ObstaclesPublisher::gate_callback, this,
                        std::placeholders::_1));
  ...
  void on_gates_transition_event(
      const lifecycle_msgs::msg::TransitionEvent::SharedPtr msg) {
    RCLCPP_INFO(this->get_logger(),
                "Received gates transition event: %s from %s",
                msg->goal_state.label.c_str(), msg->start_state.label.c_str());
    if (msg->goal_state.label == "deactivating" &&
        msg->start_state.label == "active") {
      RCLCPP_INFO(this->get_logger(), "Activating node");
  ...
  void rand_cylinder(obstacle& obs, std::vector<obstacle>& obstacles,
                     rclcpp::Time& startTime, std::mt19937& gen);
  void rand_box(obstacle& obs, std::vector<obstacle>& obstacles,
                rclcpp::Time& startTime, std::mt19937& gen);
...
std::vector<obstacle> ObstaclesPublisher::rand_obstacles() {
  ...
  std::mt19937 gen(rd());
  std::uniform_real_distribution<> size_dis(this->data.min_size,
                                            this->data.max_size);
  ...
  for (int i = 0;
       i < this->data.n_obstacles &&
       !overTime(this->get_clock(), startTime, this->data.max_timeout);
       i++) {
    ...
    if (this->data.no_cylinders) {
      ...
      obstacle_type type =
          shape(gen) == 0 ? obstacle_type::CYLINDER : obstacle_type::BOX;
      RCLCPP_INFO(this->get_logger(), "Obstacle type: %s",
                  (type == obstacle_type::BOX ? "box" : "cylinder"));
    ...
    RCLCPP_INFO(this->get_logger(),
                "Obstacle %d: x=%f, y=%f, radius=%f, dx=%f, dy=%f", i, obs.x,
                obs.y, obs.radius, obs.dx, obs.dy);
  }

  if (overTime(this->get_clock(), startTime, this->data.max_timeout)) {
    // This is not an unrecoverable error, so we don't return ERROR
    RCLCPP_ERROR(this->get_logger(),
                 "Could not find a valid position for some obstacles [%ld/%d]",
                 obstacles.size(), this->data.n_obstacles);
  }

  return obstacles;
}

void ObstaclesPublisher::spawn_and_publish_obstacles(
    std::vector<obstacle>& obstacles) {
  ...
  for (auto o : obstacles) {
    RCLCPP_INFO(this->get_logger(),
                "Publishing obstacle: %s x=%f, y=%f, radius=%f, dx=%f, dy=%f",
                (o.type == obstacle_type::BOX ? "box" : "cylinder"), o.x, o.y,
                o.radius, o.dx, o.dy);
    ...
    if (o.type == obstacle_type::BOX) {
      ...
      xml_string.assign(std::istreambuf_iterator<char>(xml_file),
                        std::istreambuf_iterator<char>());
      std::string size_string = "<size>1 1 1</size>";
      std::string size_replace_string = "<size>" + std::to_string(o.dx) + " " +
                                        std::to_string(o.dy) + " 1</size>";
      size_t pos = 0;
      ...
      std::string size_replace_string =
          "<radius>" + std::to_string(o.radius) + "</radius>";
      size_t pos = 0;
      while ((pos = xml_string.find(size_string, pos)) != std::string::npos) {
        xml_string.replace(pos, size_string.length(), size_replace_string);
        pos += size_replace_string.length();
    ...
    spawn_model(this->get_node_base_interface(), this->spawner_, xml_string,
                pose);
...
void ObstaclesPublisher::rand_cylinder(obstacle& obs,
                                       std::vector<obstacle>& obstacles,
  ...
  do {
    ...
    obs.x = 0.0;
    obs.y = 0.0;
    while (distance(obs.x, obs.y, 0.0, 0.0) < (1.5 + obs.radius) ||
           distance(obs.x, obs.y, this->gates[0].x, this->gates[0].y) <
               (1.5 + obs.radius)) {
      obs.x = x_dis(gen);
      obs.y = y_dis(gen);
    }
    if (valid_position(this->data.map_name, this->data.dx, this->data.dy, obs,
                       {obstacles, this->gates})) {
...
void ObstaclesPublisher::rand_box(obstacle& obs,
                                  std::vector<obstacle>& obstacles,
                                  rclcpp::Time& startTime, std::mt19937& gen) {
  std::uniform_real_distribution<> size_dis(this->data.min_size,
                                            this->data.max_size);
  ...
  do {
    ...
    obs.x = 0.0;
    obs.y = 0.0;
    while (distance(obs.x, obs.y, 0.0, 0.0) < 1.5 ||
           distance(obs.x, obs.y, this->gates[0].x, this->gates[0].y) < 1.5) {
      obs.x = x_dis(gen);
      obs.y = y_dis(gen);
    }
    if (valid_position(this->data.map_name, this->data.dx, this->data.dy, obs,
